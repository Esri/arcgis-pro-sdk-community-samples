...
{
    internal class RemovingSpatialIndex : Button
    {
            ...
            QueuedTask.Run(() =>
            {
                using (Geodatabase geodatabase = new Geodatabase(new FileGeodatabaseConnectionPath(new Uri(@"C:\temp\mySampleGeoDatabase.gdb"))))
                {
                    ...
                    using (FeatureClass feature = geodatabase.OpenDataset<FeatureClass>("Buildings"))
                    {
                        ...
                        RemoveSpatialIndex(schemaBuilder, featureClassDefinition);
                    }
                }
            });
        }
        ...
        public void RemoveSpatialIndex(SchemaBuilder schemaBuilder, FeatureClassDefinition featureClassDefinition)
        {
            ...
            SpatialIndexDescription spatialIndexDescription = new SpatialIndexDescription(new FeatureClassDescription(featureClassDefinition));       // Enqueue DDL operation to remove index 
            schemaBuilder.Delete(spatialIndexDescription);       // Execute delete index operation
