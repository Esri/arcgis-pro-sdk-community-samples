...
namespace SketchToolDemo
{
  ...
  class CutTool : MapTool
  {
    ...
    // overrides the tool activation to activate the esri_editing_shortcutState_Sketches state
    // this will allow the use of editing shortcuts while the tool is active
    protected override Task OnToolActivateAsync(bool active)
    {
      return QueuedTask.Run(() =>
      {
        //activate the sketching state
        FrameworkApplication.State.Activate("esri_editing_shortcutState_Sketches");
      });
    }

    // overrides the tool deactivation to deactivate the esri_editing_shortcutState_Sketches state
    protected override Task OnToolDeactivateAsync(bool active)
    {
      return QueuedTask.Run(() =>
      {
        //deactivate the sketching state
        FrameworkApplication.State.Deactivate("esri_editing_shortcutState_Sketches");
      });
    }
