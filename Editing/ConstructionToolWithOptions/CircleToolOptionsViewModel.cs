...
{
  internal class CircleToolOptionsViewModel : EmbeddableControl, IEditingCreateToolControl
  {
    ...
    public double Radius
    {
      ...
      set
      {
        if (SetProperty(ref _radius, value))
        {
          _isDirty = true;
          _isValid = true;
          // add/update the value to the tool options
          if (!ToolOptions.ContainsKey(RadiusOptionName))
            ToolOptions.Add(RadiusOptionName, value);
          else
            ToolOptions[RadiusOptionName] = value;
