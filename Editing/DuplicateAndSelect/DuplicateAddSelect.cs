...
{
  internal class DuplicateAndSelect : Button
  {
    protected override void OnClick()
    {
      try
      {
        QueuedTask.Run(() =>
        {
          ...
          var firstSelectionSet = selectedFeatures.ToDictionary().First(sel => sel.Key is BasicFeatureLayer);
          ...
          createOp.Create(inspector.MapMember, inspector);  // inspector.ToDictionary(a => a.FieldName, a => a.CurrentValue)
          if (createOp.IsEmpty)
          {
