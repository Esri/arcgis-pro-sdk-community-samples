...
{
  internal class ShowContainmentPartialButton : Button
  {
        ...
        private async Task<string> CreateDiagram(CancelableProgressorSource cps)
        {
            ...
            await QueuedTask.Run(() =>
            {
                ...
                cps.Progressor.Status = (cps.Progressor.Value * 100 / cps.Progressor.Max) + @" % Completed";
                cps.Progressor.Message = string.Format("Step 2 – Generate a diagram based on the '{0}' template", Utility.templateBasic);
                //generate a diagram
                un = Utility.GetUtilityNetworkFromMap(MapView.Active.Map);

                DiagramManager diagramManager = un?.GetDiagramManager();
                DiagramTemplate diagramtemplate = null;
                try { diagramtemplate = diagramManager.GetDiagramTemplate(Utility.templateBasic); }
                catch
                {
                    Utility.messError = Utility.messNoTemplateBasic;
                    return;
                }
                ...
                try
                {
                    ...
                    if (!myDiagram.GetDiagramInfo().CanExtend)
                    {
                        Utility.messError = Utility.messCannotExtend;
                        return;
                    }

                    cps.Progressor.Status = (cps.Progressor.Value * 100 / cps.Progressor.Max) + @" % Completed";
                    cps.Progressor.Message = string.Format("Step 3 – Extend by containment based on the '{0}' diagram", diagramName);

                    //extent only feature selected
                    myDiagram.Extend(NetworkDiagramExtendType.ExtendByContainment, listGlobalID);
                }
                catch (Exception ex)
                {
