...
{
  internal class BringForward : Button
  {
    protected async override void OnClick()
    {
      await QueuedTask.Run(() =>
      {
        ...
        var elementBringForward = graphicLayer.CanBringForward(selectedElements) ? true : false;
        if (elementBringForward)
        {
          Module1.Current.GraphicsLayerSelectedElements.FirstOrDefault().Key.BringForward(selectedElements);
        }
      });
