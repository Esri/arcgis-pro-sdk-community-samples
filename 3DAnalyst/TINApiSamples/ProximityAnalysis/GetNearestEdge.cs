...
{
  internal class GetNearestEdge : MapTool
  {
    private TinLayer _tinLayer;
    private GraphicsLayer _graphicsLayer;

    public GetNearestEdge()
    {
      IsSketchTool = true;
    ...
    protected async override Task<bool> OnSketchCompleteAsync(Geometry geometry)
    {
      ...
      await QueuedTask.Run(() => {
        ...
        {
          using (var edge = tinDataset.GetNearestEdge(selectedPoint))
          {
            var edgeGeometry = PolylineBuilderEx.CreatePolyline(edge.ToPolyline(), MapView.Active.Map.SpatialReference);
            var lineSymbol = SymbolFactory.Instance.ConstructLineSymbol(ColorFactory.Instance.GreyRGB, 1, SimpleLineStyle.Solid);
            lineSymbol.UseRealWorldSymbolSizes = true;
            _graphicsLayer.AddElement(edgeGeometry, lineSymbol, "Nearest Edge");
          }
        }       
      });
      return true;
