...
{
  internal class GetTriangleByPoint : MapTool
  {
    private TinLayer _tinLayer;
    private GraphicsLayer _graphicsLayer;

    public GetTriangleByPoint()
    {
      IsSketchTool = true;
    ...
    protected async override Task<bool> OnSketchCompleteAsync(ArcGIS.Core.Geometry.Geometry geometry)
    {
      ...
      await QueuedTask.Run(() => {
        ...
        {
          using (var triangle = tinDataset.GetTriangleByPoint(selectedPoint))
          {
            var triangleGeometry = PolygonBuilderEx.CreatePolygon(triangle.ToPolygon(), MapView.Active.Map.SpatialReference);
            ...
            _graphicsLayer.AddElement(triangleGeometry, polygonSymbol, "TINTriangle");
          }            
        }          
      });
      return true;
