...
namespace RemoveAddins
{
    ...
    internal class SampleBackstageTabViewModel : BackstageTab
    {
        ...
        internal void Del_AddIns()
        {
            ...
            foreach (var addin in AddIns)
            {
                if (addin.IsSelected)
                {
                    ...
                    // check if we have a guid folder in which case we will clean up that folder as well
                    var folder = Path.GetDirectoryName(addin.AddInFullPath);
                    if (!string.IsNullOrEmpty(folder) && folder.EndsWith("}"))
                    {
                        if (IsSubFolder(SelectedAddInPath.FullName, folder))
        ...
        private static bool IsSubFolder(string parentFolder, string subFolder)
        {
            foreach (var dirPath in Directory.GetDirectories(parentFolder))
            {
                if (dirPath.Equals(subFolder, StringComparison.CurrentCultureIgnoreCase)) return true;
            }
            return false;
        }
        ...
        {
            if (Path.GetExtension(path).Equals(".esriAddInX", StringComparison.CurrentCultureIgnoreCase))
            {
