  ...
  internal class RunModelTool : Button
  {
    protected override async void OnClick()
    {
      try
      {
        ...
        var outputFC = System.IO.Path.Combine(Project.Current.DefaultGeodatabasePath, $@"{theLayer.Name}Auto{Module1.BufferSuffix}");
        var argsModelTool = Geoprocessing.MakeValueArray(theLayer,
          outputFC,
          "60 Meters",  // Distance__value_or_field_="60 Meters",
          "PLANAR"      // Method = "PLANAR"
        );
        GPExecuteToolFlags mapOutput = GPExecuteToolFlags.AddOutputsToMap;
        // set overwrite flag           
        var env = Geoprocessing.MakeEnvironmentArray(overwriteoutput: true);
        await RunGPTool("MyModelTools.MyRedlineModelTool", argsModelTool, cps.Progressor, env, mapOutput);
      }
      catch (Exception ex)
      {
