...
{
  internal class RenderAsMultiPatchRing : Button
  {
      ...
      try
      {
        ...
        foreach (IMapPane mapPane in mapPanes)
        {
          ...
          var multiPatchPolygon = await QueuedTask.Run(async () =>
          {
            ...
            else multiPatchPoly = await Module1.MakeFishnetRingMultiPatchAsync(polygon);
            //if (Module1.Is3D)
            //{
            //  // 3D
            ...
            //{
            // 2D
            Module1.ClearGraphic(Module1.CurrentMapView);
