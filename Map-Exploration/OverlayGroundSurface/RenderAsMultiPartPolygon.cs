...
{
  internal class RenderAsMultiPartPolygon : Button
  {
    protected override async void OnClick()
    {
      try
      {
        ...
        foreach (IMapPane mapPane in mapPanes)
        {
          ...
          {
            Geometry multiPartPoly = null;
            if (Module1.HasGeometries && Module1.Geometries.Count == 0 && Module1.Geometries[0].GeometryType == GeometryType.Polygon)
            {
              multiPartPoly = Module1.Geometries[0];
            }
            else multiPartPoly = Module1.MakeFishnetPolygon(polygon);
            if (Module1.Is3D)
            {
              // 3D
              var result = await Module1.CurrentMapView.Map.GetZsFromSurfaceAsync(multiPartPoly);
              if (result.Status == SurfaceZsResultStatus.Ok)
              {
                ...
                Module1.AddOrUpdateOverlay(movedZup, Module1.GetPolygonSymbolRef());
              }
            }
            else
            {
              // 2D
              Module1.AddOrUpdateOverlay(multiPartPoly, Module1.GetPolygonSymbolRef());
            }
            return multiPartPoly;
          });
          var fishnetPolygons = new List<Geometry>() { fishnetPolygon };
