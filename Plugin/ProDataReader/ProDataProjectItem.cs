...
namespace ProDataReader
{
	...
	internal class ProDataProjectItem : CustomProjectItemBase
	{
		...
		List<ProMdbAccessDb.ProMdbAccessDb> _proMdbAccessDbs = new List<ProMdbAccessDb.ProMdbAccessDb>();
		...
		~ProDataProjectItem()
		{
			foreach (var db in _proMdbAccessDbs)
			{
				db.Close();
			}
		}
		...
		private ImageSource GetIconImage(bool bSmall)
		{
			...
			switch (SubItemType)
			{
				...
				case EnumSubItemType.ImgDirType:
					...
					break;
				case EnumSubItemType.MdbType:
					{ 
						var fcType = 0;
						var proMdbTableInfo = Specialization as ProMdbTableInfo;
						if (proMdbTableInfo != null) fcType = proMdbTableInfo.GeometryType;
						switch (fcType)
						{
							case 1:
								imgSrc = System.Windows.Application.Current.Resources[$@"GeodatabaseFeatureClassPoint{size}"] as ImageSource;
							...
							case 4:
								...
								break;
							default:
								imgSrc = new BitmapImage(new Uri($@"pack://application:,,,/ProDataReader;component/Images/Mdb{size}.png"));
								break;
						}
					}
					break;
