...
namespace ProSqlExpressDb
{
  ...
  public class ProSqlExpressDb : IDisposable
  {
    ...
    private static List<ProSqlExpressTableInfo> _lstSpatialTables = new();

    private static Dictionary<string, List<ProSqlExpressTableInfo>> SpatialTables = new();

    public ProSqlExpressDb(string sqlConnection)
    {
      SqlConnectionString = sqlConnection;
      if (!sqlConnection.Contains("TrustServerCertificate", StringComparison.OrdinalIgnoreCase))
      {
