    ...
    public class ProPluginTableTemplate : PluginTableTemplate, IDisposable, IPluginRowProvider
    {
        ...
        private void Open()
        {
            ...
            foreach (var field in parser.ReadFields())
            {
                ...
                _table.Columns.Add(new DataColumn(field_name, typeof(string)));
            }
        ...
        private List<int> ExecuteQuery(QueryFilter qf)
        {
            ...
            if (qf.ObjectIDs.Count() > 0)
            {
                ...
                result = _table.AsEnumerable().Where(
                  row => qf.ObjectIDs.Contains((int)row["OBJECTID"]))
                  .Select(row => (int)row["OBJECTID"]).ToList();
            ...
            if (emptyQuery)
            {
                ...
                result = _table.Rows.Cast<DataRow>()
                  .Select(row => (int)row["OBJECTID"]).OrderBy(x => x).ToList();
            }
            return result;
        }
