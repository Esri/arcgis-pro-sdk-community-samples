...
{
    internal class TraceButton : Button
    {
        protected override void OnClick()
        {
            ...
            try
            {
                ...
                var task = QueuedTask.Run(() =>
                    BatchTrace.Execute("Batch Trace", configuration, true, false));
                task.Wait();
                ...
                if (task.Result)
                    ...
                    MessageBox.Show("Failed", string.Format("Trace complete: {0} total seconds", timeDiff.TotalSeconds), MessageBoxButton.OK, MessageBoxImage.Warning);
            }
            catch (Exception ex)
            {
                Debug.WriteLine(ex);
